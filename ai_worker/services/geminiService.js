...
/**
 * Membedah teks naskah risalah menjadi objek terstruktur 3 Pilar.
 */
export function parsePillarsFromText(rawText) {
  ...
  if (!rawText) return defaultPillars;
  ...
  const p1Match = text.match(p1Pattern);
  const p2Match = text.match(p2Pattern);
  const p3Match = text.match(p3Pattern);

  const p1Raw = (p1Match && p1Match[1] ? p1Match[1] : '').trim();
  const p2Raw = (p2Match && p2Match[1] ? p2Match[1] : '').trim();
  const p3Raw = (p3Match && p3Match[1] ? p3Match[1] : '').trim();

  if (!p1Raw && !p2Raw && !p3Raw) {
    defaultPillars.ringkasan_utama = text;
    return defaultPillars;
  }

  defaultPillars.ringkasan_utama = p1Raw;

  if (p2Raw) {
    let rawItems = p2Raw.split(/\n(?=(?:\d+[\.\)]|\[\d{1,2}:\d{2}(?::\d{2})?\])\s+)/);
    if (rawItems.length <= 1) {
      rawItems = p2Raw.split(/\n(?=(?:[\-\*]\s+))/);
    }
    ...
    for (let itemStr of rawItems) {
      ...
      let time = null;
      const timeMatch = itemStr.match(/\[(\d{1,2}:\d{2}(?::\d{2})?)\]/);
      if (timeMatch) {
        time = timeMatch[1];
        itemStr = itemStr.replace(timeMatch[0], '').trim();
      }

      const cleanItem = itemStr.replace(/^(?:\d+[\.\)]|[\-\*])\s+/, '');
      const lines = cleanItem.split('\n');
      const firstLine = (lines[0] || '').trim();

      let topik = firstLine;
      const tMatch = firstLine.match(/^Topik:\s*(.*)$/i);
      if (tMatch) {
        topik = tMatch[1].trim();
      }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const pMatch = sub.match(/^(?:[\-\*]\s*)?Pembicara:\s*(.*)$/i);
        const uMatch = sub.match(/^(?:[\-\*]\s*)?Uraian:\s*(.*)$/i);
        if (pMatch) {
          pembicara = pMatch[1].trim();
        } else if (uMatch) {
          uraianLines.push(uMatch[1].trim());
        } else {
          uraianLines.push(sub);
        }
      }

      defaultPillars.poin_pembahasan.push({
        waktu: time,
        topik: topik,
        pembicara: pembicara,
  ...
  if (p3Raw) {
    const rawConclusions = p3Raw.split(/\n(?=(?:\d+[\.\)]|[\-\*✔✓])\s+)/u);
    for (let cStr of rawConclusions) {
      cStr = cStr.trim();
      if (!cStr) continue;
      const cleanC = cStr.replace(/^(?:\d+[\.\)]|[\-\*✔✓])\s+/u, '').trim();
      defaultPillars.kesimpulan_akhir.push(cleanC);
    }
  }
