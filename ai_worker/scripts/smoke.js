...
async function runSmokeTest() {
  ...
  if (!config.gemini.apiKey) {
    console.warn('[CONFIG] GEMINI_API_KEY tidak terdefinisi di .env. Uji inferensi AI mungkin memerlukan API Key valid.');
  } else {
    console.log(`[CONFIG] GEMINI_API_KEY terdeteksi (${config.gemini.apiKey.slice(0, 8)}...)`);
  }

  console.log(`[CONFIG] Rantai Model AI: ${config.gemini.modelChain.join(' -> ')}`);
  ...
  console.log('[AUDIO] Membuat file audio sintetis 5 detik via FFmpeg...');
  await generateSyntheticAudio(tempAudioFile);
  console.log('[AUDIO] File audio sintetis berhasil dibuat:', tempAudioFile);

  // 4. Buat record job dummy di database
  console.log('[DB] Menambahkan record job uji coba ke database...');
  ...
  console.log(`[DB] Job uji coba dibuat dengan ID #${testJobId}`);
  ...
  try {
    ...
    if (!config.gemini.apiKey) {
      console.log('[PIPELINE] Melewati panggilan Gemini AI karena API key belum diisi.');
    } else {
      console.log(`[PIPELINE] Menjalankan pipeline processJob() untuk Job #${testJobId}...`);
      await processJob(pool, job);
      console.log('[PIPELINE] Pipeline pemrosesan job selesai.');
    }
  } catch (err) {
    console.error('[ERROR] Error saat menjalankan pipeline smoke test:', err);
  } finally {
    // 5. Bersihkan data dummy
    console.log('\n[CLEANUP] Membersihkan data uji coba...');
  ...
  console.log('\n[SUCCESS] SMOKE TEST BERHASIL DISELESAIKAN!');
}

runSmokeTest().catch((err) => {
  console.error('[FATAL] Fatal Smoke Test Error:', err);
  process.exit(1);
});
