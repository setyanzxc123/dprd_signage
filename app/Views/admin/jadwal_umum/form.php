...
<div class="mb-5 flex items-center justify-between">
    <div>
        <nav class="flex items-center gap-x-2 text-xs text-slate-500 dark:text-slate-400 mb-1">
            ...
            <a href="<?= base_url('admin/jadwal-umum') ?>" class="hover:text-emerald-600 dark:hover:text-emerald-400 transition">Jadwal Umum</a>
            <i data-lucide="chevron-right" class="size-3 text-slate-400"></i>
            <span class="font-medium text-slate-800 dark:text-slate-200"><?= $isEdit ? 'Edit Jadwal' : 'Tambah Baru' ?></span>
        </nav>
...
<form action="<?= esc($action_url) ?>" method="post" enctype="multipart/form-data" class="schedule-form min-w-0 max-w-full" data-require-targets="false">
    ...
    <div class="mt-6 flex items-center justify-end gap-3 sticky bottom-4 z-10 p-3 rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-200 dark:border-slate-800 shadow-lg max-w-5xl">
        <a href="<?= base_url('admin/jadwal-umum') ?>" class="py-2 px-3.5 inline-flex items-center gap-x-2 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 shadow-xs transition">
