...
<div class="space-y-5">
    ...
    <div id="live_progress_card"
            ...
            <div class="flex items-center gap-2" id="notulen_process_controls">
                <?php if ($isCancelled): ?>
                    ...
                    <form method="post" action="<?= base_url('admin/notulen/retry/' . $job['id']) ?>">
                        <?= csrf_field() ?>
                        <button type="submit" class="py-2 px-3 sm:py-1 sm:px-2.5 inline-flex items-center gap-x-1.5 text-xs font-semibold rounded-lg bg-blue-600 text-white hover:bg-blue-700 shadow-xs transition cursor-pointer">
                ...
                <?php elseif ($isFailed): ?>
                    ...
                    <form method="post" action="<?= base_url('admin/notulen/retry/' . $job['id']) ?>">
                        <?= csrf_field() ?>
                        <button type="submit" class="py-2 px-3 sm:py-1 sm:px-2.5 inline-flex items-center gap-x-1.5 text-xs font-semibold rounded-lg bg-rose-600 text-white hover:bg-rose-700 shadow-xs transition cursor-pointer">
                ...
                <?php elseif ($isInProgress || $job['status'] === 'queued'): ?>
                    <?php if (! empty($job['cancel_requested'])): ?>
                        <button type="button" disabled class="py-2 px-3 sm:py-1 sm:px-2.5 inline-flex items-center gap-x-1.5 text-xs font-semibold rounded-lg border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 cursor-not-allowed shadow-xs opacity-90">
        ...
        <div class="relative">
            <div class="overflow-x-auto py-2.5 px-1 focus:outline-none" tabindex="0" role="region" aria-label="Tahapan Proses AI">
            ...
            <!-- Petunjuk Geser Khusus Layar Mobile -->
            <div class="sm:hidden flex items-center justify-end pr-1 pt-1 text-[11px] text-slate-400 dark:text-slate-500">
                <span class="inline-flex items-center gap-1">
                    <i data-lucide="chevrons-right" class="size-3.5"></i>
        ...
        <?php if ($isInProgress || $job['status'] === 'queued'): ?>
            <div class="p-2.5 rounded-xl border border-slate-200 bg-slate-50 dark:bg-slate-800/40 dark:border-slate-800 text-xs flex items-center gap-2 text-slate-600 dark:text-slate-300 mt-2">
                <i data-lucide="info" class="size-4 shrink-0 text-slate-400"></i>
                <span>Proses AI berjalan di latar belakang. Halaman ini dapat Anda tutup sewaktu-waktu.</span>
            </div>
        <?php elseif ($isCancelled): ?>
            <div class="p-2.5 rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-900/40 text-xs flex items-center gap-2 text-amber-800 dark:text-amber-300 mt-2">
                <i data-lucide="pause" class="size-4 shrink-0 text-amber-600 fill-current"></i>
                <span>Proses AI dijeda. Klik <strong>Lanjutkan Proses</strong> di bagian atas untuk melanjutkan.</span>
            </div>
        <?php elseif ($isFailed): ?>
            <div class="p-2.5 rounded-xl border border-rose-200 bg-rose-50 dark:bg-rose-950/20 dark:border-rose-900/40 text-xs flex items-center gap-2 text-rose-800 dark:text-rose-300 mt-2">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-xs dark:bg-slate-900 dark:border-slate-800 p-5 space-y-4">
        
        <!-- Header: Judul Rapat & Tombol Navigasi -->
        ...
        <!-- Detail Metadata Rapat Berbaris Rapi -->
        <div class="pt-3 border-t border-slate-100 dark:border-slate-800/60 space-y-2 text-xs">
            <div class="flex items-baseline gap-2">
                <span class="w-32 sm:w-36 shrink-0 text-slate-500 dark:text-slate-400 font-medium">Tanggal</span>
                <span class="text-slate-400 dark:text-slate-500 select-none shrink-0">:</span>
                <span class="min-w-0 flex-1 font-medium text-slate-800 dark:text-slate-200"><?= esc($tanggalRapat) ?></span>
            </div>
            <div class="flex items-baseline gap-2">
                <span class="w-32 sm:w-36 shrink-0 text-slate-500 dark:text-slate-400 font-medium">Waktu &amp; Durasi</span>
                <span class="text-slate-400 dark:text-slate-500 select-none shrink-0">:</span>
                <span class="min-w-0 flex-1 font-medium text-slate-800 dark:text-slate-200">
                    ...
                    <span class="text-slate-500 dark:text-slate-400 font-normal">(Durasi Rekaman: <?= esc($durationFormatted) ?>)</span>
                </span>
            </div>
            <div class="flex items-baseline gap-2">
                <span class="w-32 sm:w-36 shrink-0 text-slate-500 dark:text-slate-400 font-medium">Ruangan / Tempat</span>
                <span class="text-slate-400 dark:text-slate-500 select-none shrink-0">:</span>
                <span class="min-w-0 flex-1 font-medium text-slate-800 dark:text-slate-200"><?= esc($schedule['ruangan'] ?? 'Ruang Rapat Paripurna DPRD Provinsi Sulawesi Tengah') ?></span>
            </div>
            <div class="flex items-baseline gap-2">
                <span class="w-32 sm:w-36 shrink-0 text-slate-500 dark:text-slate-400 font-medium">Unit Kerja / Komisi</span>
                <span class="text-slate-400 dark:text-slate-500 select-none shrink-0">:</span>
                <span class="min-w-0 flex-1 font-medium text-slate-800 dark:text-slate-200"><?= esc(! empty($schedule['unit']) && $schedule['unit'] !== '-' ? $schedule['unit'] : ($job['jadwal_type'] === 'banmus' ? 'Badan Musyawarah' : 'Seluruh Fraksi & Komisi')) ?></span>
            </div>
        </div>

    </div>

    <div id="tab_panel_risalah" class="w-full bg-white border border-slate-200 rounded-2xl p-5 sm:p-7 shadow-xs dark:bg-slate-900 dark:border-slate-800 space-y-6">
            
            <?php if ($minutes && ! empty($minutes['ringkasan_eksekutif'])): ?>
                <!-- Action Bar & Verification Status -->
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between items-start gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
                    <!-- Status Verifikasi -->
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <?php if (! $isFinal): ?>
                            ...
                            <form method="post" action="<?= base_url('admin/notulen/finalize/' . $minutes['id']) ?>"
                                  ...
                                  class="m-0 flex-1 sm:flex-initial inline-flex">
                                <?= csrf_field() ?>
                                <button type="submit" class="w-full justify-center py-2.5 px-3.5 sm:py-1.5 sm:px-2.5 inline-flex items-center gap-x-1.5 text-xs font-semibold rounded-lg bg-blue-600 text-white hover:bg-blue-700 shadow-xs transition cursor-pointer">
                                    <i data-lucide="check-check" class="size-3.5"></i>
                                    <span class="sm:hidden">Finalisasi</span>
                                    <span class="hidden sm:inline">Sahkan / Finalisasi</span>
                                </button>
                            </form>
                        <?php else: ?>
                            <form method="post" action="<?= base_url('admin/notulen/unfinalize/' . $minutes['id']) ?>"
                                  ...
                                  class="m-0 flex-1 sm:flex-initial inline-flex">
                                <?= csrf_field() ?>
                                <button type="submit" class="w-full justify-center py-2.5 px-3.5 sm:py-1.5 sm:px-2.5 inline-flex items-center gap-x-1.5 text-xs font-semibold rounded-lg border border-amber-300 bg-amber-50 text-amber-700 hover:bg-amber-100 dark:bg-amber-950/40 dark:border-amber-800 dark:text-amber-300 transition cursor-pointer">
...
<?= esc($minutes['ringkasan_eksekutif']) ?>
                ...
                <div id="risalah_edit_mode" class="space-y-4 hidden">
                    <form id="notulen_minutes_form" method="post" action="<?= base_url('admin/notulen/update-minutes/' . $minutes['id']) ?>" class="space-y-4">
                        ...
                        <div class="space-y-0">
                            <div class="flex items-center justify-between text-xs font-bold text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-800/60 py-2 px-3.5 rounded-t-xl border border-b-0 border-slate-200 dark:border-slate-700">
                                <div class="flex items-center gap-2">
                                    <i data-lucide="sparkles" class="size-3.5 text-blue-600 dark:text-blue-400"></i>
                                    <span>I. RINGKASAN UTAMA</span>
                                </div>
                                <span class="hidden sm:inline text-[11px] font-normal text-slate-500 dark:text-slate-400">Latar belakang dan gambaran umum</span>
                            </div>
                            <textarea id="section_ringkasan"
                                      name="section_ringkasan"
                                      rows="6"
                                      class="notulen-editor-section py-3 px-3.5 block w-full border border-slate-200 dark:border-slate-700 rounded-b-xl rounded-t-none text-base sm:text-sm font-sans leading-relaxed outline-none focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:bg-slate-900 dark:text-slate-200"
                        ...
                        <div class="space-y-0">
                            <div class="flex items-center justify-between text-xs font-bold text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-800/60 py-2 px-3.5 rounded-t-xl border border-b-0 border-slate-200 dark:border-slate-700">
                                <div class="flex items-center gap-2">
                                    <i data-lucide="list-ordered" class="size-3.5 text-blue-600 dark:text-blue-400"></i>
                                    <span>II. POIN-POIN PEMBAHASAN</span>
                                </div>
                                <span class="hidden sm:inline text-[11px] font-normal text-slate-500 dark:text-slate-400">Pokok bahasan dan tanggapan</span>
                            </div>
                        ...
                        <div class="space-y-0">
                            <div class="flex items-center justify-between text-xs font-bold text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-800/60 py-2 px-3.5 rounded-t-xl border border-b-0 border-slate-200 dark:border-slate-700">
                                <div class="flex items-center gap-2">
                                    <i data-lucide="check-square" class="size-3.5 text-blue-600 dark:text-blue-400"></i>
                                    <span>III. KESIMPULAN &amp; KEPUTUSAN AKHIR</span>
                                </div>
                                <span class="hidden sm:inline text-[11px] font-normal text-slate-500 dark:text-slate-400">Butir kesepakatan dan rekomendasi</span>
                            </div>
                        ...
                        <div class="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 pt-2 border-t border-slate-200 dark:border-slate-800">
                            <div class="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                                <span id="last_saved_time"></span>
                            </div>
                            <div class="flex items-center gap-2 w-full sm:w-auto">
                                <button type="button" id="btn_cancel_edit_risalah" class="flex-1 sm:flex-initial justify-center py-2.5 px-3.5 sm:py-2 sm:px-3 inline-flex items-center text-xs font-medium rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 shadow-xs transition cursor-pointer">
                                    Tutup / Batal
                                </button>
                                <button type="submit" id="btn_save_draft" class="flex-1 sm:flex-initial justify-center py-2.5 px-3.5 sm:py-2 sm:px-3 inline-flex items-center gap-x-1.5 text-xs font-semibold rounded-lg bg-blue-600 text-white hover:bg-blue-700 shadow-xs transition cursor-pointer">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div class="lg:col-span-7 bg-white border border-slate-200 rounded-2xl shadow-xs dark:bg-slate-900 dark:border-slate-800 p-5 space-y-3.5">
            ...
            <!-- Detail Metadata Berkas Audio -->
            <div class="pt-3 border-t border-slate-100 dark:border-slate-800/60 space-y-2 text-xs">
                <div class="flex items-baseline gap-2">
                    <span class="w-28 shrink-0 text-slate-500 dark:text-slate-400 font-medium">Nama Berkas</span>
                    <span class="text-slate-400 dark:text-slate-500 select-none">:</span>
                    <span class="font-mono text-[11px] text-slate-800 dark:text-slate-200 truncate" title="<?= esc($job['audio_filename']) ?>"><?= esc($job['audio_filename']) ?></span>
                </div>
                <div class="flex items-baseline gap-2">
                    <span class="w-28 shrink-0 text-slate-500 dark:text-slate-400 font-medium">Ukuran &amp; Tipe</span>
                    <span class="text-slate-400 dark:text-slate-500 select-none">:</span>
                    <span class="text-slate-800 dark:text-slate-200 font-medium"><?= round($job['audio_size'] / (1024 * 1024), 2) ?> MB (<?= esc(strtoupper(pathinfo((string) $job['audio_filename'], PATHINFO_EXTENSION) ?: 'WAV')) ?>)</span>
                </div>
                <div class="flex items-baseline gap-2">
                    <span class="w-28 shrink-0 text-slate-500 dark:text-slate-400 font-medium">Durasi Rekaman</span>
                    <span class="text-slate-400 dark:text-slate-500 select-none">:</span>
                    <span class="font-mono text-[11px] text-slate-800 dark:text-slate-200 font-medium"><?= esc($durationFormatted) ?></span>
                </div>
                <div class="flex items-baseline gap-2">
                    <span class="w-28 shrink-0 text-slate-500 dark:text-slate-400 font-medium">Segmen AI</span>
                    <span class="text-slate-400 dark:text-slate-500 select-none">:</span>
                    <span class="text-slate-800 dark:text-slate-200 font-medium"><?= (int) ($job['total_chunks'] ?? 1) ?> segmen chunk</span>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-5 bg-white border border-slate-200 rounded-2xl shadow-xs dark:bg-slate-900 dark:border-slate-800 p-5 space-y-3.5">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <?php if ($hasAudioFile): ?>
                    <a href="<?= base_url('admin/notulen/audio/' . $job['id']) ?>" download="<?= esc($job['audio_filename']) ?>"
                       class="notulen-action-card border border-slate-200 dark:border-slate-800 rounded-xl p-3 flex flex-col items-center justify-center gap-1 text-center bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/60 shadow-xs transition cursor-pointer">
                        ...
                        <span class="text-xs font-bold text-slate-800 dark:text-slate-200">Unduh Audio</span>
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Master Audio (<?= esc(strtoupper(pathinfo((string) $job['audio_filename'], PATHINFO_EXTENSION) ?: 'WAV')) ?>)</span>
                    </a>
                <?php else: ?>
                    <div role="button" aria-disabled="true" class="border border-slate-200 dark:border-slate-800 rounded-xl p-3 flex flex-col items-center justify-center gap-1 text-center bg-slate-50 dark:bg-slate-800/30 text-slate-400 dark:text-slate-600 select-none">
                        <i data-lucide="download" class="size-5"></i>
                        <span class="text-xs font-bold">Unduh Audio</span>
                        <span class="text-[11px] font-medium">Tidak Tersedia</span>
                    </div>
                <?php endif; ?>
                ...
                <?php if ($isCompleted && ! empty($transcripts['full_text'])): ?>
                    <a href="<?= base_url('admin/notulen/download-transcript/' . $job['id']) ?>"
                       class="notulen-action-card border border-slate-200 dark:border-slate-800 rounded-xl p-3 flex flex-col items-center justify-center gap-1 text-center bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/60 shadow-xs transition cursor-pointer">
                        ...
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Teks Lengkap (.txt)</span>
                    </a>
                <?php else: ?>
                ...
                <?php if ($isCompleted && $minutes && ! empty($minutes['ringkasan_eksekutif'])): ?>
                    <a href="<?= base_url('admin/notulen/export-pdf/' . $minutes['id']) ?>" target="_blank"
                       class="notulen-action-card border border-slate-200 dark:border-slate-800 rounded-xl p-3 flex flex-col items-center justify-center gap-1 text-center bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/60 shadow-xs transition cursor-pointer">
                        ...
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Dokumen Resmi (.pdf)</span>
                    </a>
                <?php else: ?>
                ...
                <button type="button" data-hs-overlay="#modal_riwayat_proses"
                    ...
                    <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Log Audit Sistem</span>
                </button>
...
<!-- Modal riwayat proses dan audit log -->
    ...
    <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-300 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center">
        <div class="w-full flex flex-col bg-white border border-slate-200 shadow-xl rounded-2xl pointer-events-auto dark:bg-slate-900 dark:border-slate-800">
            <div class="flex justify-between items-center py-3.5 px-4 sm:px-6 border-b border-slate-200 dark:border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400">Informasi teknis dan jejak eksekusi proses AI</p>
                    </div>
                </div>
            ...
            <div class="p-4 sm:p-6 space-y-3 text-xs">
                <div class="flex justify-between items-baseline border-b border-slate-100 dark:border-slate-800 pb-1.5">
                ...
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-0.5 sm:gap-2 border-b border-slate-100 dark:border-slate-800 pb-1.5">
                ...
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-0.5 sm:gap-2 border-b border-slate-100 dark:border-slate-800 pb-1.5">
                    ...
                    <span class="font-semibold text-slate-800 dark:text-slate-200 break-words" id="ai_model_meta_text"><?= esc($aiModelLabel ?? \App\Libraries\Notulen\NotulenService::formatAiModelLabel($job['ai_model'] ?? null)) ?></span>
                </div>
